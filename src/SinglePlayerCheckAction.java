 ...
 */
public class SinglePlayerCheckAction implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (value.length() >= 4) {
            if (value.startsWith("hair")) {
                Compare =  String.valueOf(SinglePlayerGuessWhoGame.getGuilty().getAttribute("hairColor").equals(value.substring(4)));
                trait = value.substring(4);
                value = "hairColor";
            } else if (value.startsWith("eyes")) {
                Compare = String.valueOf(SinglePlayerGuessWhoGame.getGuilty().getAttribute("eyeColor").equals(value.substring(4)));
                trait = value.substring(4);
                value = "eyeColor";
            } else if (value.startsWith("gend")) {
                Compare = String.valueOf(SinglePlayerGuessWhoGame.getGuilty().getAttribute("gender").equals(value.substring(4)));
                trait = value.substring(4);
                value = "gender";
            } else {
                Compare = SinglePlayerGuessWhoGame.getGuilty().getAttribute(value);
                trait = "true";
            }
        } else {
            Compare = SinglePlayerGuessWhoGame.getGuilty().getAttribute(value);
        ...
        for (int i = 0; i < SinglePlayerGuessWhoGame.getTheDeck().getTotalCards(); i++) {
            if (!String.valueOf(SinglePlayerGuessWhoGame.getTheDeck().getSuspect(i).getAttribute(value).equals(trait)).equals(Compare)) {
                SinglePlayerGuessWhoGame.getWindow().flipCard(i);
            }
        }
